...
async function loadAffairDetail(affairId) {
    ...
    try {
        ...
        const affair = await window.GristAPI.fetchRecord('Affaires', affairId);

        // Récupération des membres liés
        const members = await window.GristAPI.fetchRelatedRecords('Affaires_Membres', 'AffaireId', affairId);
        const membersDetails = await Promise.all(
            members.map(async (am) => {
                const member = await window.GristAPI.fetchRecord('Membres', am.MembreId);
